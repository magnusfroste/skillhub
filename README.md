...
| Hand over a document (the file goes by ticket -- `curl -T file https://<store>/deliver/<ticket>` -- never by a token the agent has to copy) | `upload_url` gives two curl lines — the file, and its `pdftotext -layout` output — then `load_text`: the text is held verbatim with page numbers, searched by words and by meaning, a hit names the page, `read` gives it whole or by `pages`, and a quotation is the document's own words |
...
caretaker's answer — whether search by meaning is on (`index`), and what is on the
noticeboard (`questions`).
