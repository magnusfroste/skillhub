...
| MCP server — nineteen tools, identity from the verified header; indexer | `volumes/functions/skillhub`, `volumes/functions/embed` | container start |
...
| Door | Key | Behind it |
|---|---|---|
| `https://<domain>/skillhub` | any `MCP_KEY_NN` (consumer `agent_NN`), or the service key | the nineteen tools; identity from the key |
...
- **A deploy that changes the tool list does not reach a running agent.** Hermes reads
  ...
  for an hour). The server answers nineteen tools while the agent still sees an older count, and
